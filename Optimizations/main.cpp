...
JsonConfig config("/Users/macbook/CLionProjects/lab_7_Arya/tosave.json");
GLuint texture[1];
...
int main() {
    ...
    FPS *fps = new FPS();
#ifdef MAXIMAL_SPLIT
    if(config.count_of_height_segments > MAXIMAL_SPLIT)
        config.count_of_height_segments = MAXIMAL_SPLIT;
    if(config.count_of_width_segments > MAXIMAL_SPLIT)
        config.count_of_width_segments = MAXIMAL_SPLIT;
#endif
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (config.move_flag) {
#ifdef ANIMATION
            if(ellipsoid->count_of_height_segments != 50 || ellipsoid->count_of_width_segments != 50)
                ellipsoid->remake_ellipsoid(50, 50);
#endif
            flag_stop = true;
...
#ifdef ANIMATION
        if(!config.move_flag && (ellipsoid->count_of_height_segments != config.count_of_height_segments || ellipsoid->count_of_width_segments != config.count_of_width_segments))
            ellipsoid->remake_ellipsoid(config.count_of_width_segments, config.count_of_height_segments);
#endif
        last = current;
        if (config.texture)
...
void key_callback(GLFWwindow *window, int key, int scancode, int action, int mods) {
    ...
    if (key == GLFW_KEY_9 && (action == GLFW_PRESS || action == GLFW_REPEAT))
        config.save("/Users/macbook/CLionProjects/lab_7_Arya/tosave.json");
}
void make_perspective() {
